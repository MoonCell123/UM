...
class HamiltonianNetwork(nn.Module):
    ...
    def _rotation_evolution(self, x):
        """基于旋转矩阵的演化（保持能量守恒）"""
        q = x[:, :self.half_dim]
        p = x[:, self.half_dim:]

        # 使用固定角度的旋转
        theta = self.delta_t * self.time_steps
        cos_theta = torch.cos(torch.tensor(theta, device=x.device))
        sin_theta = torch.sin(torch.tensor(theta, device=x.device))

        # 在q-p空间中进行旋转
        q_new = cos_theta * q - sin_theta * p
        p_new = sin_theta * q + cos_theta * p

        return torch.cat([q_new, p_new], dim=1)

    def _fourier_evolution(self, x):
        """基于傅里叶变换的演化"""
        # 分离 q 和 p
        q = x[:, :self.half_dim]
        p = x[:, self.half_dim:]

        # 对q进行FFT，对p进行IFFT，然后交换
        q_fft = torch.fft.fft(q, dim=-1).real
        p_ifft = torch.fft.ifft(p, dim=-1).real

        # 哈密顿演化：位置和动量的耦合
        evolution_factor = self.delta_t * self.time_steps
        q_evolved = q + evolution_factor * p_ifft
        p_evolved = p - evolution_factor * q_fft

        return torch.cat([q_evolved, p_evolved], dim=1)

    def _simple_hamiltonian_evolution(self, x):
        """最简单的哈密顿演化 - 仅使用基本数学运算"""
        q = x[:, :self.half_dim]
        p = x[:, self.half_dim:]

        # 简单的耦合振子模型
        # dq/dt = p, dp/dt = -ω²q (ω = 1)
        omega_sq = 1.0
        dt = self.delta_t

        for _ in range(self.time_steps):
            # 使用Verlet积分保持辛几何性质
            q_new = q + dt * p - 0.5 * dt * dt * omega_sq * q
            p_new = p - dt * omega_sq * (q + q_new) * 0.5
    ...
    def forward(self, x):
        ...
        elif self.evolution_type == 'rotation':
            evolved_main = self._rotation_evolution(main_features)
        elif self.evolution_type == 'fourier':
            evolved_main = self._fourier_evolution(main_features)
        else:  # 'simple'
            evolved_main = self._simple_hamiltonian_evolution(main_features)
...
class NewModel_3(nn.Module):
    def __init__(self, conf, instance_loss_fn=nn.CrossEntropyLoss()):
        ...
        hamiltonian_evolution_type = getattr(conf, 'hamiltonian_evolution_type', 'simple')
