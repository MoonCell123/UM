...
class S4Model(nn.Module):
    ...
    def forward(self, x):
        ...
        # 将所有批次的patch合并处理
        x_flat = x.reshape(-1, D)  # 形状为(B*N, D)
        x_flat = self._fc1(x_flat)  # 形状为(B*N, 512)

        # 重塑为原始批次结构
        x = x_flat.reshape(B, N, 512)  # 形状为(B, N, 512)

        # 应用S4D块，它可以处理形状为(B, N, 512)的输入
        x = self.s4_block(x)  # 形状为(B, N, 512)

        # 在patch维度上取最大值
        x = torch.max(x, dim=1)[0]  # 形状为(B, 512)
